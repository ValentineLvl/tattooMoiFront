...
function SearchScreen(props) {
        ...
        AsyncStorage.getItem("dataUserToken", function (error, data) {
            ...
            if (data) {
                const findUser = async () => {
                    const reqFind = await fetch(`http://172.17.1.32:3000/client-data?token=${data}`)
    ...
    const handlePress = async (tattooStyle) => {
        ...
        let rawResponse = await fetch(`http://172.17.1.32:3000/search-tattoo?styleList=${tattooStyle}`)
    ...
    const onSearchInput = async (name) => {
        ...
        let rawResponse = await fetch(`http://172.17.1.32:3000/search-tattoo?firstName=${name}`)
        let response = await rawResponse.json()
